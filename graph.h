...
private:
  ...
  map<VertexT, map<VertexT,WeightT>>graphData;
  //
...
bool _LookupEdge(VertexT from, VertexT to) const{
    if(graphData.count(from)>0){
        map<VertexT, WeightT> lilMap;
        lilMap = graphData.at(from);
        if (lilMap.count(to)>0){
            return true;
        }
        else{
            return false;
...
public:
  ...
  {
    return static_cast<int>(this->graphData.size());
  }
...
*/
    ...
    map <VertexT, WeightT> lilMap = graphData[from];
    lilMap[to] = weight;
    graphData[from] = lilMap;
    return true;
  }
...
*/
   ...
   if (_LookupVertex(from) == false){
    ...
    map <VertexT, WeightT> lilMap = graphData.at(from);
    if (lilMap.count(to)==1){
    weight = lilMap.at(to);
    return true;
    }
    else{
  ...
  vector<VertexT> getVertices() const
  {
    ...
    vector<VertexT> vertice;
    for (auto& i : graphData){
        vertice.push_back(i.first);
    }
    return vertice;
  }
...
/*    for (int row = 0; row < this->NumVertices(); ++row)
    ...
    map<VertexT, WeightT> lilMap;
    for (auto vertx : graphData){
        lilMap = graphData.at(vertx.first);
        output<<" "<<vertx.first<<": ";
        for(auto edgs : lilMap){
            output<<"("<<vertx.first<<","<<edgs.first<<","<<edgs.second<<")"<<" ";
        }
        output<<endl;
    }
    output << "**************************************************" << endl;
